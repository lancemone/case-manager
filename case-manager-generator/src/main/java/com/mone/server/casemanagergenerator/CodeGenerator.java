...
import com.mone.server.casemanagergenerator.config.*;
import com.mone.server.casemanagergenerator.constants.GeneratorConstant;
import com.mone.server.casemanagergenerator.exception.GeneratorException;
...
public class CodeGenerator {
    ...
    public void init(String tableName, GeneratorProperties generatorProperties) {
        ...
        log.debug("parentPackage: " + parentPackage);
        log.debug("setParentPackagePath: " + generatorProperties.getParentPackagePath());
    ...
    public void generator(String tableName, String pkIdName, GeneratorProperties generatorProperties) {
        ...
        if (StringUtils.isBlank(outputDir)) {
            String rootProjectFile = System.getProperty("user.dir");
        ...
        log.debug("dataSourceConfig: \n" + dataSourceConfig.toString());
        // 设置元数据自定义查询
        DbType dbType = dataSourceConfig.getDbType();
        if (DbType.MYSQL == dbType) {
            // 设置MySQL元数据自定义查询
            dataSourceConfig.setDbQuery(new MoneMySqlQuery());
        }
