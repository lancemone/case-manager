package com.mone.server.casemanagerframework.aop;
...
import com.mone.server.casemanagerframework.aop.log.annotation.OperationLogIgnore;
...
import com.mone.server.casemanagerframework.common.bean.ClientInfo;
...
import com.mone.server.casemanagerframework.util.AnsiUtil;
import com.mone.server.casemanagerframework.util.ClientInfoUtil;
import com.mone.server.casemanagerframework.util.DateUtil;
import com.mone.server.casemanagerframework.util.UUIDUtil;
...
/**
 * @Description Controller Aop 抽象类
 * 获取响应结果信息
 * <p>
 * 日志输出类型：print-type
 * 1. 请求和响应分开，按照执行顺序打印
...
public abstract class BaseLogAop {
    ...
    @Autowired
    protected CmsOperationLogService operationLogService;
    ...
    protected static ThreadLocal<OperationLogInfo> operationLogInfoThreadLocal = new ThreadLocal<>();
     ...
     * Aop日志配置
     */
    protected AopProperties.LogAopConfig logAopConfig;

    /**
     * 日志打印类型
     */
    protected AopProperties.LogPrintType logPrintType;
     ...
     * requestId生成类型
     */
    protected AopProperties.RequestIdType requestIdType;

    /**
     * Aop操作日志配置
     */
    protected AopProperties.OperationLogConfig operationLogConfig;
...
//            }
    ...
    protected void handleAfterReturn(Object result, Exception exception) {
        ...
        // 获取OperationLogInfo
        OperationLogInfo operationLogInfo = operationLogInfoThreadLocal.get();
    ...
    private void handleOperationLogInfo(Method method) {
        ...
        operationLogInfoThreadLocal.set(operationLogInfo);
    }
...
//    }
    ...
    protected void handleRequestId(RequestInfo requestInfo) {
        ...
        if (AopProperties.RequestIdType.IDWORK == requestIdType) {
            requestId = IdWorker.getIdStr();
        } else if (AopProperties.RequestIdType.UUID == requestIdType) {
    ...
    protected void handleRequestInfo(RequestInfo requestInfo) {
        ...
        if (AopProperties.LogPrintType.NONE == logPrintType) {
            return;
        }
        // 获取请求信息
        String requestInfoString = formatRequestInfo(requestInfo);
        // 如果打印方式为顺序打印，则直接打印，否则，保存的threadLocal中
        if (AopProperties.LogPrintType.ORDER == logPrintType) {
    ...
    protected void handleResponseResult(Object result) {
        ...
        if (result != null && result instanceof ApiResult) {
            ...
            if (AopProperties.LogPrintType.ORDER == logPrintType) {
                ...
                if (AopProperties.LogPrintType.LINE == logPrintType) {
                    printRequestInfoString(requestInfoString);
                    printResponseResult(code, responseResultString);
                } else if (AopProperties.LogPrintType.MERGE == logPrintType) {
...
//        }
    ...
    protected void saveSysOperationLog(RequestInfo requestInfo, OperationLogInfo operationLogInfo, Object result, Exception exception) {
        try {
            ...
            // 操作日志
            CmsOperationLog cmsOperationLog = new CmsOperationLog();
            // 设置操作日志信息
            if (operationLogInfo != null) {
                ...
                cmsOperationLog.setModule(operationLogInfo.getModule())
            ...
            // 设置请求参数信息
            if (requestInfo != null) {
                cmsOperationLog.setIp(requestInfo.getIp())
                ...
                // 设置参数字符串
//                cmsOperationLog.setParam(Jackson.toJsonStringNonNull(requestInfo.getParam()));
                // User-Agent
                ClientInfo clientInfo = ClientInfoUtil.get(requestInfo.getUserAgent());
                if (clientInfo != null) {
                    cmsOperationLog.setBrowserName(clientInfo.getBrowserName())
                            .setBrowserVersion(clientInfo.getBrowserversion())
                            .setOsName(clientInfo.getOsName())
                            .setMobile(clientInfo.isMobile())
                            .setDeviceName(clientInfo.getDeviceBrand())
                            .setDeviceModel(clientInfo.getDeviceModel());
                }
                // 设置IP区域
                IpAddressEntity ipAddress = ipAddressService.getByIp(requestInfo.getIp());
                if (ipAddress != null) {
                    requestInfo.setIpAddress(ipAddress);
                    cmsOperationLog.setArea(ipAddress.getArea()).setOperator(ipAddress.getOperator());
                }
            }
            ...
            if (result != null && result instanceof ApiResult) {
                ApiResult<?> apiResult = (ApiResult<?>) result;
                apiResult.getCode();
                cmsOperationLog.setSuccess(apiResult.isSuccess())
                        .setCode(apiResult.getCode())
                        .setMessage(apiResult.getMessage());
            }

            // 设置当前登录信息
//            cmsOperationLog.setUserId(LoginUtil.getUserId()).setUserName(LoginUtil.getUsername());
            ...
            if (exception != null) {
                ...
                if (exception instanceof BaseGlobalException) {
                    BaseGlobalException springBootPlusException = (BaseGlobalException) exception;
                    errorCode = springBootPlusException.getErrorCode();
                }
                // 异常字符串长度截取
                cmsOperationLog.setSuccess(false)
                        .setCode(errorCode)
                        .setExceptionMessage(exceptionMessage)
                        .setExceptionName(exception.getClass().getName());
            }
            // 保存日志到数据库
            operationLogService.saveCmsOperationLog(cmsOperationLog);
...
//            }
    ...
    protected void saveSysLoginLog(RequestInfo requestInfo, OperationLogInfo operationLogInfo, Object result, Exception exception) {
        try {
            ...
            // 保存登录登出日志
            if (flag) {
                CmsLoginLog cmsLoginLog = new CmsLoginLog();
                cmsLoginLog.setType(type);
                // 设置异常信息
                if (exception != null) {
                    Integer errorCode = null;
                    String exceptionMessage = exception.getMessage();
                    ...
                    // 异常字符串长度截取
                    cmsLoginLog.setCode(errorCode).setExceptionMessage(exceptionMessage);
                }

                // TODO:判断登录登出结果
//                if (result != null && result instanceof ApiResult) {
//                    ApiResult<?> apiResult = (ApiResult<?>) result;
//                    cmsLoginLog.setSuccess(apiResult.isSuccess()).setCode(apiResult.getCode());
...
//                                    cmsLoginLog.setToken(tokenMd5);
//                                }
//                            }
//                        }
//                    } else {
//                        cmsLoginLog.setExceptionMessage(apiResult.getMessage());
//                    }
//                }

                // TODO:设置请求参数信息
//                if (requestInfo != null) {
//                    cmsLoginLog.setIp(requestInfo.getIp()).setRequestId(requestInfo.getRequestId());
//                    // 设置登录用户名
//                    if (LOGIN_TYPE == type) {
...
//                            cmsLoginLog.setUsername(username);
//                        }
//                    } else if (LOGOUT_TYPE == type) {
//                        String username = JwtUtil.getUsername(requestInfo.getToken());
//                        cmsLoginLog.setUsername(username);
//                        // 设置登出token
//                        cmsLoginLog.setToken(requestInfo.getTokenMd5());
//                    }

                    // User-Agent
                    String userAgent = requestInfo.getUserAgent();
                    if (StringUtils.isNotBlank(userAgent)) {
                        cmsLoginLog.setUserAgent(StringUtils.substring(userAgent, ZERO, MAX_LENGTH));
                    }
                    ClientInfo clientInfo = ClientInfoUtil.get(userAgent);
                    if (clientInfo != null) {
                        cmsLoginLog.setBrowserName(clientInfo.getBrowserName())
                                .setBrowserVersion(clientInfo.getBrowserversion())
                                .setOsName(clientInfo.getOsName())
                                .setMobile(clientInfo.isMobile())
                                .setDeviceBrand(clientInfo.getDeviceBrand())
                                .setDeviceModel(clientInfo.getDeviceModel());
                    }
                    IpAddressEntity ipAddress = requestInfo.getIpAddress();
                    if (ipAddress == null) {
                        ipAddress = ipAddressService.getByIp(requestInfo.getIp());
                    }
                    if (ipAddress != null) {
                        cmsLoginLog.setArea(ipAddress.getArea()).setOperator(ipAddress.getOperator());
                    }
                    // 保存登录日志
                    loginLogService.saveCmsLoginLog(cmsLoginLog);
                }
        } catch (Exception e) {
